...
function handleGoToUnfinishedNotes(navigate: NavigateFunction, dispatch: Dispatch) {
  const unfinishedNotesRoute = getRoute(Pages.UnfinishedNotes);
  dispatch(modalsActions.hideAllModals());
  unfinishedNotesRoute && navigate(unfinishedNotesRoute);
  playSound(AgentSounds.END_OF_UTTERANCE);
  dispatch(agentActions.setAgentState(SukiAgentState.READY));
}

export default handleGoToUnfinishedNotes;
