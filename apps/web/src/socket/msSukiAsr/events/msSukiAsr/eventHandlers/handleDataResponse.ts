...
const MS_SUKI_ASR_RESPONSE_PROCESS = new Process()
...
function handleDataResponse(msSukiAsrResponse: DialogResponse, { socket, getMsSukiAsrDialog }: any) {
  const { intent, notification, transfer_to_cursor, action_response } = msSukiAsrResponse;
  MS_SUKI_ASR_RESPONSE_PROCESS.run({
    action_response,
    getMsSukiAsrDialog,
    intent,
    notification,
